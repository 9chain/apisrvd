...
def internal_error(data=None):
    return json_error(-32603, "Internal error", data)

def invalid_request_error(data=None):
    return json_error(-32600, "Invalid Request", data)

def receipt_error(data=None):
    return json_error(-32010, "Receipt creation error", data)

def invalid_param_error(data=None):
    return json_error(-32602, "Invalid params", data)

def missing_chainhead_error(data=None):
    return json_error(-32009, "Missing Chain Head", data)

def blockchain_error(data=None):
    return json_error(-33000, "Fail to validate in btc", data)

def method_not_found_error(data=None):
    return json_error(-32601, "Method not found", data)

def parse_error(data=None):
    return json_error(-32700, "Parse error", data)